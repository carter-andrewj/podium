...
import MiniLoader from '../../components/immutableComponent';
...
// FORM VALIDATION
	...
	triggerValidation(event) {
		...
		validationTimer = setTimeout(() => this.validate(false), 500)
	}
...
// RENDER
	...
	render() {
		...
		const filled = this.getState("valid")

		return (
			<div
				className={open ?
					"register-capture register-capture-open" :
					"register-capture register-capture-closed"
				}
				...
				<div className={open ?
					...
					<form onSubmit={this.next.bind(this)}>
						...
						<div
							className={
								!open ? "card register-button register-button-hide" :
								error ? "card register-button register-button-error" :
								loading ? "card register-button register-button-loading" :
								filled ? "card register-button register-button-on" :
									"card register-button register-button-off"
							}
							...
							{error ?
								<i className="fas fa-exclamation register-button-icon" />
								:
								loading ?
									<MiniLoader size={0.8} />
									:
									<i className="fas fa-user-plus register-button-icon" />
							}
						</div>
						<button
							type="submit"
