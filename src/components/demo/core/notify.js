...
class Notify extends Component {
	...
	setColors() {
		...
		state.color.main = "rgb(" + rgb.join(",") + ")";
		state.color.bg = "rgba(" + rgb.join(",") + ",0.2)";
		this.setState(state);
	}
	...
	render() {
		...
		return (
			<div
				...
				style={{
					background: this.state.color.bg,
					color: this.state.color.main
				}}>
				<div
					className="notif-title"
					style={{ background: this.state.color.main }}>
					{this.props.title}
				</div>
				...
				<div
					...
					style={{ color: this.state.color.main }}>
					{stage}
				</div>
			</div>
		);
